...
print("=" * 60)
print("Analyzing which 12-minute cycle creates the low in bullish hours")
print("=" * 60)
...
# Identify bullish hourly candles
hourly_df['is_bullish'] = hourly_df['close'] > hourly_df['open']
bullish_count = hourly_df['is_bullish'].sum()
print(f"Created {len(hourly_df):,} hourly candles")
print(f"Found {bullish_count:,} bullish hours ({bullish_count/len(hourly_df)*100:.1f}%)")

# For each bullish hour, identify which 12-min cycle created the low
print("Analyzing which 12-minute cycle sets the low in bullish hours...")
...
# Create a lookup dictionary for hourly lows
hourly_lows = {}
for _, row in hourly_df[hourly_df['is_bullish']].iterrows():
    key = (row['hour_start'], row['symbol'])
    hourly_lows[key] = (row['low'], row['hour_of_day'])

# Group data by hour_start and symbol
grouped = df.groupby(['hour_start', 'symbol'])

for (hour_start, symbol), group in grouped:
    # Check if this is a bullish hour
    key = (hour_start, symbol)
    if key not in hourly_lows:
        continue
        
    hourly_low, hour_of_day = hourly_lows[key]
    
    # Find the first 12-minute candle that has the low
    low_candles = group[group['low'] == hourly_low]
    if not low_candles.empty:
        cycle = low_candles.iloc[0]['12min_cycle']
...
print(f"Successfully analyzed {processed_count:,} bullish hours")
...
print("=" * 50)
print("\nDistribution of 12-minute cycles creating the low in bullish hourly candles:")
...
print(f"\nThe 12-minute cycle with the highest probability of creating the low in a bullish hourly candle:")
...
if cme_total > 0:
    ...
    print("\nDistribution of 12-minute cycles during CME trading hours:")
...
if regular_total > 0:
    ...
    print("\nDistribution of 12-minute cycles during Regular Market Hours:")
